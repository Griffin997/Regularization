...
file_oi = "NESMA_cropped_slice5.mat"#"AIC_triTest.mat"#"NESMA_cropped_slice5.mat"
folder_oi = "BLSA_1742_04_MCIAD_m41"#"AIC_tests"#"BLSA_1742_04_MCIAD_m41"
specific_name = 'slice_oi'#"AIC_triTest"#'slice_oi' - this is important if the data strux has an internal name
...
if model_selection:
    seriesTag = (seriesTag + "AIC_filter" + "_")
...
def calculate_AIC(RSS, popt, sigma):

    # AIC = 1/TDATA.shape[0] * (RSS + np.log(TDATA.shape[0]) * popt.shape[0]*(sigma)**2)
    # The extra parameter in the len(popt) is due to the offset term also estimated
    AIC = len(TDATA) * np.log(RSS/len(TDATA)) + (len(popt)+1)*2
...
def AIC_filter(data):
    ...
    AIC_G_biX = calculate_AIC(RSS_biX, G_biX_off_params, data[0]/curve_SNR)
    AIC_G_moX = calculate_AIC(RSS_moX, G_moX_off_params, data[0]/curve_SNR)

    return AIC_G_moX < AIC_G_biX, G_moX_off_params, AIC_G_moX
...
def main_estimator(i_voxel, full_brain_data, func):
    ...
    if model_selection:
        AIC_boolean, G_moX_off_params, RSS_moX = AIC_filter(data)
    else:
        AIC_boolean = False
